...
#    #@position(_el)
  ...
  send_message: ->
    ...
    text_field.val('')
    date = new Date
    gmt_time = date.toGMTString()
    console.log 'Send message =>'
    data =  { content: content, time_at: date.toLocaleString().split(' ')[1],  gmt_time: gmt_time}
    req = jQuery.getJSON(@site_config_url , {format: "json", data: data})
